...
export function CreatedAtModal() {
    const [certificiations, setCertificiations] = useState<Game[]>([])
            ...
            <Dialog.Title className='text-3xl font-black'>
              ...
              <form onSubmit={handleCreatedAd} className='mt-8 flex flex-col gap-4'>
                <div className='flex flex-col gap-2'>
                  <label className='font-semibold' htmlFor="certificiation">Qual a certificação que você está estudando?</label>
                    <Select.Root name="certificiation" >
                        <Select.Trigger className="bg-zinc-900 py-3 px-4 rounded text-sm placeholder:text-zinc-200 flex justify-between items-center">
                            <Select.Value placeholder={`Selecione a certificação`} />
                            <Select.Icon>
                                <CaretDown size={24} />
                            </Select.Icon>
                        </Select.Trigger>
                            ...
                            <Select.ScrollUpButton className="flex align-center justify-center h-6 bg-zinc-700 pt-2 text-white cursor-default" >
                              ...
                              <Select.Viewport>
                                    {certificiations.map(certificiation => {
                                      return (
                                        <Select.Item className="select-none py-4 px-8 w-[25rem] flex items-center hover:bg-blue-500 " key={certificiation.id} value={certificiation.id}  >
                                            <Select.ItemText>{certificiation.title}</Select.ItemText>
                ...
                <div className='flex flex-col gap-2'>
                  ...
                  <Input name="name" id="name" placeholder="Digite como prefere ser chamado(a) durante o estudo?" maxLength={80}  />
                </div>
                  ...
                  <div className='flex flex-col gap-2'>
                    <label htmlFor="weekDays">Quando costuma estudar?</label>
                        ...
                        <ToggleGroup.Root className="grid grid-cols-4 gap-2" value={weekDays} type="multiple" onValueChange={setWeekDays}>
                            <ToggleGroup.Item  value="0"                          className={`w-8 h-8 rounded ${weekDays.includes('0') ? 'bg-blue-500' : 'bg-zinc-900'}`} title="Domingo">D</ToggleGroup.Item>
                            
                            <ToggleGroup.Item  value="1"                          className={`w-8 h-8 rounded ${weekDays.includes('1') ? 'bg-blue-500' : 'bg-zinc-900'}`} title="Segunda">S</ToggleGroup.Item>
                            
                            <ToggleGroup.Item  value="2"                          className={`w-8 h-8 rounded ${weekDays.includes('2') ? 'bg-blue-500' : 'bg-zinc-900'}`} title="Terça">T</ToggleGroup.Item>
                            
                            <ToggleGroup.Item  value="3"                          className={`w-8 h-8 rounded ${weekDays.includes('3') ? 'bg-blue-500' : 'bg-zinc-900'}`} title="Quarta">Q</ToggleGroup.Item>
                            
                            <ToggleGroup.Item  value="4"                          className={`w-8 h-8 rounded ${weekDays.includes('4') ? 'bg-blue-500' : 'bg-zinc-900'}`} title="Quinta">Q</ToggleGroup.Item>
                            
                            <ToggleGroup.Item  value="5"                          className={`w-8 h-8 rounded ${weekDays.includes('5') ? 'bg-blue-500' : 'bg-zinc-900'}`} title="Sexta">S</ToggleGroup.Item>
                            
                            <ToggleGroup.Item  value="6"                          className={`w-8 h-8 rounded ${weekDays.includes('6') ? 'bg-blue-500' : 'bg-zinc-900'}`} title="Sábado">S</ToggleGroup.Item>
                        </ToggleGroup.Root>
                ...
                <footer className="mt-3 flex justify-end gap-2 sm:gap-4">
                  ...
                  <button className="bg-blue-500 px-5 h-8 sm:h-12 rounded-md font-semibold flex items-center gap-3 hover:bg-blue-600" type="submit">
